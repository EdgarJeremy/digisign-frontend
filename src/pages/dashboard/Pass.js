...
import { Table, Icon, Divider, InputGroup, Input, Modal, Button } from 'rsuite';
    ...
    async fetch() {
        ...
        const letters = await models.Letter.collection({
            ...
            where: {
                ...
                ...(() => {
                    if (user.type === 'Bagian Umum') {
                        return {
                            type: 'Reguler'
                        }
                    } else if (user.type === 'Bagian Hukum') {
                        return {
                            type: 'Hukum'
                        }
                    } else if (['Asisten 1', 'Asisten 2', 'Asisten 3'].indexOf(user.type) !== -1) {
                        return {
                            '$division.asst$': user.type
                        }
                    } else {
                        return {}
                    }
            ...
            <div className="contain" style={{ padding: 14 }}>
                ...
                <div className="panel">
                    ...
                    <Table
                        ...
                        <Table.Column flexGrow={1}>
                            <Table.HeaderCell>Lihat Dokumen Final</Table.HeaderCell>
                            <Table.Cell>
                                {(row) => (
                                    <a href="#" onClick={(e) => {
                                        e.preventDefault();
